  ...
  <div v-if="user" class="bg-white p-8 rounded-lg shadow-md">
    <div class="flex items-center space-x-6 mb-8">
      ...
      <div>
        ...
        <div class="mt-2 flex items-center space-x-2">
          ...
          <span class="text-2xl font-semibold">{{ parseFloat(user.account?.reputation_score || 0).toFixed(1) }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div class="border p-4 rounded-lg">
        ...
        <p class="text-gray-600">Deals Count: {{ user.account?.deals_count || 0 }}</p>
        <p class="text-gray-600">Feedback: {{ user.account?.positive_feedback_percent || 0 }}%</p>
      </div>
      <div class="border p-4 rounded-lg">
    ...
    <div class="mt-8 flex space-x-4">
      <button @click="handleRating" :disabled="ratingLoading" class="flex-1 bg-green-600 text-white py-3 rounded-lg font-bold hover:bg-green-700 disabled:bg-green-300 transition">
        <span v-if="ratingLoading" class="animate-spin mr-2">↻</span>
        {{ ratingLoading ? 'Processing...' : 'Leave Rating' }}
      </button>
      <NuxtLink to="/search" class="px-6 py-3 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition">
        Back to Search
      </NuxtLink>
    </div>
  </div>
